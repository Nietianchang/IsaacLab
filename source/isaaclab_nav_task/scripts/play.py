...
def find_latest_checkpoint(log_path: str, checkpoint_pattern: str = "model_.*.pt") -> str:
    ...
    # Walk run directories newest-first and return the first one that actually
    # contains a matching checkpoint. This skips freshly-created empty run dirs
    # (e.g. a new tensorboard/params dir created at launch time) that would
    # otherwise shadow the most recent run that has real checkpoints.
    ...
    raise ValueError(
        f"No checkpoint files matching '{checkpoint_pattern}' found in any run directory under: {log_path}"
    )
