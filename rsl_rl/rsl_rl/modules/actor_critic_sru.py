...
class ActorCriticSRU(nn.Module):
    ...
    def _debug_mem(self, stage: str):
        if not torch.cuda.is_available() or not getattr(self, "debug_memory", False):
            return
        allocated = torch.cuda.memory_allocated() / 1e9
        reserved = torch.cuda.memory_reserved() / 1e9
        print(f"[ActorSRU Mem] {stage} allocated={allocated:.3f}GB reserved={reserved:.3f}GB")
    ...
    def process_actor_input(self, observations: torch.Tensor, masks, hidden_states) -> torch.Tensor:
        ...
        self._debug_mem("actor_input_start")
        ...
        self._debug_mem("after_actor_split")
        ...
        self._debug_mem("after_actor_attention")
