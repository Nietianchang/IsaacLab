...
class OnPolicyRunner:
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):
        ...
        # Resolve CUDA device for memory stats to avoid reading from a different current device.
        cuda_mem_device = None
        if torch.cuda.is_available():
            device_obj = self.device if isinstance(self.device, torch.device) else torch.device(self.device)
            if device_obj.type == "cuda":
                cuda_mem_device = device_obj.index if device_obj.index is not None else torch.cuda.current_device()
        ...
        for it in range(start_iter, tot_iter):
            ...
            # Rollout
            _mem_before_hl = _mem_after_hl = _mem_after_ll = 0.0
            with torch.no_grad():
                for i in range(self.num_steps_per_env):
                    if cuda_mem_device is not None and it == 0 and i == 0:
                        _mem_before_hl = torch.cuda.memory_allocated(cuda_mem_device) / 1024**3

                    actions = self.alg.act(obs, critic_obs)

                    if cuda_mem_device is not None and it == 0 and i == 0:
                        _mem_after_hl = torch.cuda.memory_allocated(cuda_mem_device) / 1024**3

                    obs, rewards, dones, infos = self.env.step(actions)

                    if cuda_mem_device is not None and it == 0 and i == 0:
                        _mem_after_ll = torch.cuda.memory_allocated(cuda_mem_device) / 1024**3
                        print(f"[GPU Mem] HighLevel model: {_mem_before_hl:.3f}GB -> {_mem_after_hl:.3f}GB  (delta={_mem_after_hl - _mem_before_hl:+.3f}GB)")
                        print(f"[GPU Mem] LowLevel model:  {_mem_after_hl:.3f}GB -> {_mem_after_ll:.3f}GB  (delta={_mem_after_ll - _mem_after_hl:+.3f}GB)")
            ...
            _mem_before_update = torch.cuda.memory_allocated(cuda_mem_device) / 1024**3 if cuda_mem_device is not None and it == 0 else 0.0

            if cuda_mem_device is not None and it == 0:
                free_bytes, total_bytes = torch.cuda.mem_get_info(cuda_mem_device)
                free_gb = free_bytes / 1024**3
                total_gb = total_bytes / 1024**3
                allocated_gb = torch.cuda.memory_allocated(cuda_mem_device) / 1024**3
                reserved_gb = torch.cuda.memory_reserved(cuda_mem_device) / 1024**3
                non_pytorch_gb = max(0.0, (total_gb - free_gb) - allocated_gb)
                print(
                    f"[GPU Mem] pre-update | free={free_gb:.3f}GB | allocated={allocated_gb:.3f}GB "
                    f"| reserved={reserved_gb:.3f}GB | non_pytorch_est={non_pytorch_gb:.3f}GB"
                )
            ...
            if cuda_mem_device is not None and it == 0:
                _mem_after_update = torch.cuda.memory_allocated(cuda_mem_device) / 1024**3
                print(f"[GPU Mem] Gradient update: {_mem_before_update:.3f}GB -> {_mem_after_update:.3f}GB  (delta={_mem_after_update - _mem_before_update:+.3f}GB)")
            if self.is_mdpo:
            ...
            # Print GPU memory usage every iteration; track delta from baseline to detect leaks
            if cuda_mem_device is not None:
                allocated = torch.cuda.memory_allocated(cuda_mem_device) / 1024**3
                reserved = torch.cuda.memory_reserved(cuda_mem_device) / 1024**3
                total = torch.cuda.get_device_properties(cuda_mem_device).total_memory / 1024**3
                if it == start_iter:
                    _mem_baseline = allocated
                delta_from_start = allocated - _mem_baseline
                print(f"[GPU Mem] iter={it} | allocated={allocated:.3f}GB | reserved={reserved:.3f}GB | total={total:.3f}GB | leak_delta={delta_from_start:+.4f}GB")
